														...
														<nav class="navbar navbar-light bg-faded m-b-30 p-10">
													        <div class="card-block">
									                        	<div class=" waves-effect waves-light m-r-10 v-middle issue-btn-group">
									                            	<button rel="<%=contextPath %>/leave/new_leave_type" id="new_leave_type" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">Create</span></button>
													                        ...
													                        <div class="table-responsive">
													                            <table id="issue-list-table" class="table dt-responsive width-100">
												                                	...
												                                	<tbody class="text-left">
													                                	<% 
													                                	List<Leave_type> leave_typeList=(List<Leave_type>)request.getAttribute("leave_typeList");
													                                		if(leave_typeList!=null&&!leave_typeList.isEmpty()){
													                                			int idx_num=1;
													                                			for (Leave_type leave_type : leave_typeList) {
													                                				
													                                			
													                                	%>
												                                		<tr>
																							<td class="txt-primary"><%=idx_num %></td>
															                                <td><%=leave_type.getLt_name() %></td>
															                                <td><%=leaveService.getLt_is_unpaid_value(leave_type.getLt_is_unpaid()) %></td>
															                                <td><%=leaveService.getLt_allc_mode_value(leave_type.getLt_allc_mode())%></td>
															                                <td><%=leave_type.getLt_start_date() %></td>
															                                <td><%=leave_type.getLt_end_date() %></td>
																					        <td class="center text-center">
																								<%-- <button href="javscript:void(0);" rel="<%=contextPath %>/view_leave_type/<c:out value='${obj.lt_id}' />" class="btn-sm btn-info btn-icon view_item" data-toggle="tooltip" data-placement="top" title="View" ><i class="icofont icofont-eye-alt"></i></button>
										                                                   		<button href="javscript:void(0);" rel="<%=contextPath %>/edit_leave_type/<c:out value='${obj.lt_id}' />" class="btn-sm btn-warning btn-icon edit_item" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icofont icofont-edit"></i></button>
										                                                  		<button href="javscript:void(0);" rel="<%=contextPath %>/delete_leave_type/<c:out value='${obj.lt_id}' />" class="btn-sm btn-danger btn-icon alert-confirm remove_item" data-toggle="tooltip" data-placement="top" title="Delete/Remove"><i class="icofont icofont-archive"></i></button>
										                                                  		 --%>
										                                                  	
										                                                  		<button href="javscript:void(0);" rel="<%=contextPath %>/leave/view_leave_type?lt_id=<%=leave_type.getLt_id() %>" class="btn btn-info btn-mini b-none view_item" data-toggle="tooltip" data-placement="top" title="View" ><i class="icofont icofont-eye-alt"></i></button>
								                                                   				<button href="javscript:void(0);" rel="<%=contextPath %>/leave/edit_leave_type?lt_id=<%=leave_type.getLt_id() %>" class="btn btn-warning btn-mini b-none edit_item" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icofont icofont-edit"></i></button>
								                                                  				<button href="javscript:void(0);" rel="<%=contextPath %>/leave/delete_leave_type?lt_id=<%=leave_type.getLt_id() %>" class="btn btn-danger btn-mini b-none alert-confirm remove_item" data-toggle="tooltip" data-placement="top" title="Delete/Remove"><i class="icofont icofont-archive"></i></button>
										                                                  	
										                                                  	</td>
										                                                  	
										                                                  	<td><%=leave_type.getLt_description() %></td>
																						</tr>
																                        <%} }else{ %>
																		                        <tr>
							                                	...
							                                	%><!-- Invoice list card start -->
														                <div class="col-sm-6">
														                    <div class="card card-border-primary">
														                        <div class="card-header">
														                            <h5><%=leave_type.getLt_name() %></h5>
														                        </div>
														                        <div class="card-block">
														                            <div class="row">
														                                <div class="col-sm-6">
														                                    <ul class="list list-unstyled">
														                                        <li>Is Unpaid: &nbsp;<%=leaveService.getLt_is_unpaid_value(leave_type.getLt_is_unpaid()) %></li>
														                                        <li>Take leave as: <span class="text-semibold"><%=leaveService.getLt_take_leave_value(leave_type.getLt_take_leave())%></span></li>
														                                        <li>Allocation Mode: <span class="text-semibold"><%=leaveService.getLt_allc_mode_value(leave_type.getLt_allc_mode())%></span></li>
														                                        <li>Validation: <span class="text-semibold"><%=leaveService.getLt_valid_by_value(leave_type.getLt_valid_by())%></span></li>
														                                    </ul>
														                                </div>
														                                <div class="col-sm-6">
