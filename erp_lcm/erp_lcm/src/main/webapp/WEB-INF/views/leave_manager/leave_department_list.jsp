														...
														<nav class="navbar navbar-light bg-faded m-b-30 p-10">
													        <div class="card-block">
									                        	<div class=" waves-effect waves-light m-r-10 v-middle issue-btn-group">
									                            	<button rel="<%=contextPath %>/leave/new_leave_department" id="new_leave_department" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">Create</span></button>
													                        ...
													                        <div class="table-responsive">
													                            <table id="issue-list-table" class="table dt-responsive width-100">
												                                	...
												                                	<tbody class="text-left">
													                                	<% 
													                                	List<Leave_Department> leave_department_list=(List<Leave_Department>)request.getAttribute("leave_department_list");
													                                		if(leave_department_list!=null&&!leave_department_list.isEmpty()){
													                                			int idx_num=1;
													                                	for (Leave_Department leave_department : leave_department_list) {
													                                				%>
												                                		<tr>
																							<td class="txt-primary"><%=idx_num++%></td>
															                                <td><%=leave_department.getLd_leave_type() %></td>
															                               <td><%=leave_department.getLd_duration() %></td>
															                                <td><%=leave_department.getLd_display_name() %></td>
															                                  <td><%=leave_department.getLd_manager() %></td>
															                                    <td><%=leave_department.getLd_parent_department() %></td>
																					       		 <td class="center text-center">
																								 <button href="javscript:void(0);" rel="<%=contextPath %>/view_leave_department/<c:out value='${obj.ld_id}' />" class="btn-sm btn-info btn-icon view_item" data-toggle="tooltip" data-placement="top" title="View" ><i class="icofont icofont-eye-alt"></i></button>
										                                                   		<button href="javscript:void(0);" rel="<%=contextPath %>/edit_leave_department/<c:out value='${obj.ld_id}' />" class="btn-sm btn-warning btn-icon edit_item" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icofont icofont-edit"></i></button>
										                                                  		<button href="javscript:void(0);" rel="<%=contextPath %>/delete_leave_department/<c:out value='${obj.ld_id}' />" class="btn-sm btn-danger btn-icon alert-confirm remove_item" data-toggle="tooltip" data-placement="top" title="Delete/Remove"><i class="icofont icofont-archive"></i></button>
														                    ...
														                    <div class="card card-border-primary">
														                            ...
														                            <div class="row">
														                                ...
														                                <div class="col-sm-6">
														                                    <ul class="list list-unstyled text-right">
														                                        <li>Leave Type: <span class="text-semibold"><%=leave_department.getLd_leave_type()%></span></li>
														                                         <li>Duration: <span class="text-semibold"><%=leave_department.getLd_duration()%></span></li>
														                                         <li>Display Name: <span class="text-semibold"><%=leave_department.getLd_display_name()%></span></li>
														                                        <li>Manager: <span class="text-semibold"><%=leave_department.getLd_manager() %></li>
														                                        <li>Parent Department: <span class="text-semibold"><%=leave_department.getLd_parent_department()%></span></li>
														                                    </ul>
														                                </div>
														                            </div>
														                        </div>
														                        <div class="card-footer">
														                            ...
														                            <div class="task-board m-0">
														                                <button href="javscript:void(0);" rel="<%=contextPath %>/leave/view_leave_department?Ld_id=<%=leave_department.getLd_id() %>" class="btn btn-info btn-mini b-none view_item" data-toggle="tooltip" data-placement="top" title="View" ><i class="icofont icofont-eye-alt"></i></button>
								                                                   		<button href="javscript:void(0);" rel="<%=contextPath %>/leave/edit_leave_department?Ld_id=<%=leave_department.getLd_id() %>" class="btn btn-warning btn-mini b-none edit_item" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icofont icofont-edit"></i></button>
								                                                  		<button href="javscript:void(0);" rel="<%=contextPath %>/leave/delete_leave_department?Ld_id=<%=leave_department.getLd_id() %>" class="btn btn-danger btn-mini b-none alert-confirm remove_item" data-toggle="tooltip" data-placement="top" title="Delete/Remove"><i class="icofont icofont-archive"></i></button>
	   ...
	   <!-- add footer page here -->
		...
		<script>
		    ...
		    $(document).ready(function() {
		        ...
		        $('#new_leave_department').on('click', function(){
		          	var loc=$(this).attr('rel');
		           	console.log(loc);
		           	$("#page_body").load(loc);
				});
