							...
							<nav class="navbar navbar-light bg-faded m-b-30 p-10 " style="background-color:#f3f3f3">
						        <div class="card-block">
		                        	<div class=" waves-effect waves-light m-r-10 v-middle issue-btn-group">
		                            	<button rel="<%=contextPath %>/ehs/new_occupational_programs" id="new_occupational_programs" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">Create</span></button>
		                            	<button rel="<%=contextPath %>/ehs/new_occupational_law" id="new_occupational_law" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">OHSAS LAW</span></button>
		                            	<button rel="<%=contextPath %>/ehs/new_first_aid" id="new_first_aid" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">FIRST AID TRAINING</span></button>
			                	...
			                	<div class="">
			                		...
			                		<div>
					                    <div class="card-header">
					                        <h5> Occupational Health Programs</h5>
					                            ...
					                            <table id="issue-list-table" class="table dt-responsive width-100">
													 <thead class="text-left">
													    ...
													    <tbody class="text-left">
						                                	<tr>
						                                		<% 
						                                	List<Ehs_Training> ehs_training =(List<Ehs_Training>)request.getAttribute("training_list");
						                                		if(ehs_training !=null&&!ehs_training.isEmpty()){
						                                			int idx_num=1;
						                                			for (Ehs_Training training : ehs_training ) {
						                                							                                			
						                                	%>
						                                	
						                                	<tr>
					                                		 <td class="txt-primary"><%=idx_num++ %></td>
											                 <td><%=training.getTraining_name() %></td>
											                 <td><%=training.getTraining_date() %>  	
											                 <td><%=training.getTraining_aspect() %></td>
											                 <td><%=training.getTraining_rat()%></td>
											            	<%--  <td><%=training.getTraining_des() %></td> --%>
				                                        	<%--  <td><%=action.getAction_id()%></td> --%>
			                                              	<td class="center text-center">
																 <button href="javscript:void(0);" rel="<%=contextPath %>/ehs/view_occupational_programs?training_id=<%=training.getTraining_id()%>" class="btn-sm btn-info btn-icon view_item" data-toggle="tooltip" data-placement="top" title="Training Details" ><i class="icofont icofont-eye-alt"></i></button> 
				                                             	<%-- <button href="jacvscript:void(0);" rel="<%=contextPath %>/ehs/complaint_occupational_programs" class="btn-sm btn-warning btn-icon edit_item" data-toggle="tooltip" data-placement="top" title="Complaint"><i class="icofont icofont-edit"></i></button>
				                                             	<button href="javscript:void(0);" rel="<%=contextPath %>/ehs/firstaid_occupational_programs" class="btn-sm btn-info btn-icon view_item" data-toggle="tooltip" data-placement="top" title="First-Aid"><i class="icofont icofont-eye-alt"></i></button> --%>
...
<script>
		     ...
		     $(document).ready(function() {
		        ...
		        $('#new_occupational_programs').on('click', function() {
		           var loc=$(this).attr('rel');
		           $("#page_body").load(loc);
				});
		        ...
		        $('#new_occupational_law').on('click', function() {
			           var loc=$(this).attr('rel');
			           $("#page_body").load(loc);
					});
		       
		        $('#new_first_aid').on('click', function() {
			           var loc=$(this).attr('rel');
			           $("#page_body").load(loc);
					});
