							...
							<nav class="navbar navbar-light bg-faded m-b-30 p-10" style="background-color:#f3f3f3">
						        <div class="card-block">
		                        	<div class=" waves-effect waves-light m-r-10 v-middle issue-btn-group">
		                            	<button rel="<%=contextPath %>/ehs/new_product" id="new_product" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">Create</span></button>
 		                            	<button rel="<%=contextPath %>/ehs/mail_ehs" id="mail_ehs" type="button" class="btn btn-sm btn-success btn-new-tickets waves-effect waves-light m-r-15 m-b-5 m-t-5"><i class="icofont icofont-paper-plane"></i><span class="m-l-10">Mail</span></button>
		                    		...
		                    		<%-- <form id="search_hazard" method="post" action="<%=contextPath%>/ehs/mng_product">
			                    		<div class="card-block p-t-10">
			                    			<input type="hidden" name="search_hazard" value="true"/>
						                    <div class="input-group">
						                    	<lable> Chemical Name&nbsp </lable>
						                    ...
						                    <div class="input-group">
						                    	<Button type="submit" class="input-group-addon" id="btn">
						                        	Search&nbsp;<i class="icofont icofont-search"></i>
					                        	</button>
						                    </div>
						                </div>
					                </form> --%>
					                            ...
					                            <table id="issue-list-table" class="table dt-responsive width-100">
													 <thead class="text-left">
													    ...
													    <tbody class="text-left">
						                                	<% 
						                                	List<Ehs_Product> product_list =(List<Ehs_Product>)request.getAttribute("product_list");
						                                		if(product_list !=null&&!product_list.isEmpty()){
						                                			int idx_num=1;
						                                			for (Ehs_Product products : product_list ) {
						                                							                                			
						                                	%>
						                                	<tr>
					                                		 <td class="txt-primary"><%=idx_num++ %></td>
											                 <td><%=products.getProduct_name()%></td>
											                 <td><%=products.getProd_code()%></td>
											                 <td><%=products.getManufacturing_date()%></td>
											                 <td><%=products.getExpiry_date() %></td>
											                  <td >
				                                        	  <button href="javscript:void(0);" rel="<%=contextPath %>/ehs/view_product?product_id=<%=products.getProduct_id()%>" class="btn-sm btn-info btn-icon view_item" data-toggle="tooltip" data-placement="top" title="View" ><i class="icofont icofont-eye-alt"></i></button>
				                                             	 <button href="jacvscript:void(0);" rel="<%=contextPath %>/ehs/edit_product?product_id=<%=products.getProduct_id()%>" class="btn-sm btn-warning btn-icon edit_item" data-toggle="tooltip" data-placement="top" title="Edit"><i class="icofont icofont-edit"></i></button>
											                 </td>
															</tr>
									                        <%} } else{ %>
									                        <tr>
