												...
												<div class="col-xl-12 col-lg-12 filter-bar">
													...
													<div class="card-block">
														<div
															...
															<button rel="<%=contextPath %>/purchase/new_po"
																id="new_po" type="button"
																		...
																		<div class="table-responsive">
																			<table id="issue-list-table"
																				...
																				<tbody class="text-left">
																					<% 
						                                	List<RequestQuotation> po_List=(List<RequestQuotation>)request.getAttribute("POLIST");
						                                		if(po_List!=null&&!po_List.isEmpty()){
						                                			int idx_num=1;
						                                			for (RequestQuotation po : po_List) {
						                                							                                			
						                                	%>
																					<tr>
																						<td class="txt-primary"><%=idx_num++ %></td>
																						<td><%=po.getRfq_odr_date() %></td>
																						<td><%=po.getCustomer() %></td>
																						<td><%=po.getRfq_odr_date()%></td>
																						<td><%=helper.getStatusLable(po) %></td>
																						<td><%=po.getNet_amount()%></td>

																						<td class="center text-center"><%=purchaseService.getAllButton(po,contextPath).toString() %>
																						</td>
																						<td><%=po.getRfq_extra_note()%></td>
																						<td><%=po.getRfq_extra_note()%></td>
	...
	<script>
		    ...
		    $(document).ready(function() {
		        ...
		        $('.remove_item').on('click',function(){
		            ...
		            swal({
		                 title: "Are you sure?",
		                 text: "Your will not be able to recover this Purchase Order!",
		                 type: "warning",
		                 ...
		                 $.ajax({
		                     ...
		                     success: function (data) {
		                    	 if (data=='true') {
		                     		...
		                     		swal("Deleted!", "Purchase Order has been deleted.", "success");
								 } 
		                     },
